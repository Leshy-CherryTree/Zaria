...
public class SystemProperties implements Serializable
{
    ...
    public enum Country
    {
	    ...
	    STATIUS												("BONAIRE, SINT EUSTATIUS AND SABA","BQ"),
	    ...
	    NORTH_KOREA											("DEMOCRATIC PEOPLE'S REPUBLIC OF KOREA","KP"),
	    SOUTH_KOREA											("REPUBLIC OF KOREA","KR"),
	    KUWAIT												("KUWAIT","KW"),
	    ...
	    PALAU												("PALAU","PW"),
	    PALESTINE											("OCCUPIED PALESTINIAN TERRITORY","PS"),
	    PANAMA												("PANAMA","PA"),
	    ...
	    SAINT_HELENA											("ASCENSION AND TRISTAN DA CUNHA SAINT HELENA","SH"),
	    SAINT_KITTS_AND_NEVIS								("SAINT KITTS AND NEVIS","KN"),
	    SAINT_LUCIA											("SAINT LUCIA","LC"),
	    FRENCH_SAINT_MARTIN									("SAINT MARTIN (FRENCH PART)","MF"),
	    ...
	    SAMOA												("SAMOA","WS"),
	    SAN_MARINO											("REPUBLIC OF SAN MARINO","SM"),
	    ...
	    DUTCH_SAINT_MARTIN									("SAINT MARTIN (DUTCH PART)","SX"),
	    ...
	    UNITED_KINGDOM										("UNITED KINGDOM OF GREAT BRITTAIN AND NORTHERN IRELAND","GB"),
	...
	static void initOS()
	{
		...
		for(int i = 0 ; i < devices.length ; i++)
		{
			displays[i] = new DisplayInfo(
					devices[i].getDisplayMode().getBitDepth(),
					devices[i].getDisplayMode().getWidth(),
					devices[i].getDisplayMode().getHeight(),
					devices[i].getDisplayMode().getRefreshRate());
		}

		DebugManager.trace("Operating system: " + operatingSystem + " " + operatingSystemString + " " + architecture);
		DebugManager.trace("Max avaialable memory: " + maxMemory + " bytes");
		DebugManager.trace("Java: " + javaVendor + " " + javaVendor + " " + javaArchicture);
		DebugManager.trace("Virtual Machine: " + javaVMVendor + " " + javaVMName + " " + javaVMVersion);
		DebugManager.trace("User Name: " + userName);
		DebugManager.trace("User Country: " + country);
		DebugManager.trace("User Language: " + language);
	}
	...
	private static OSName parseOSName(String s)
	{
		...
		if(check.contains("linux"))
			return OSName.LINUX;
		else if(check.contains("windows"))
			return OSName.WINDOWS;
		else if(check.contains("mac"))
			return OSName.MAC_OS;
		else if(check.contains("bsd"))
			return OSName.BSD;
		else if(check.contains("sunos"))
			return OSName.SOLARIS;
		else
			return OSName.UNKOWN;
	}
	...
	{
		if(s.contains("64"))
			return Architecture.BIT_64;
		else
			return Architecture.BIT_32;
	}
	...
	public static String getAnynomousString()
	{
		...
		ret += "Maximum amount of memory: " + maxMemory + " bytes\n";
